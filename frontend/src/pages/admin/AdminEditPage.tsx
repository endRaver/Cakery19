...
import { filter, isEmpty, map, times } from "lodash";
...
const AdminEditPage = () => {
  ...
  const { isLoading, currentProduct, fetchProductsById, handleUpdateProduct } = useProductStore();
  ...
  useEffect(() => {
    if (currentProduct) {
      setVariantAmount(currentProduct.variants.length || 1);
      setNewProduct({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEmpty(images) && isEmpty(newProduct.imageUrl))
    ...
    formData.append("imageUrl", JSON.stringify(newProduct.imageUrl));
        ...
        <div className="mt-8 space-y-4">
          ...
          <div className="flex h-[250px] w-full max-w-[780px] overflow-x-auto bg-primary-50 p-4">
            <div className="flex h-full flex-nowrap items-center gap-2">
              {isEmpty(images) && isEmpty(newProduct.imageUrl) && (
              ...
              {!isEmpty(newProduct.imageUrl) &&
                map(newProduct.imageUrl, (image, index) => (
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button
              ...
              disabled={isLoading}
            >
              {isLoading ? <Loader2 className="mx-2 mt-1 animate-spin" /> : "Update Product"}
            </Button>
          </div>
        </div>
      </form>
