...
import { sendPasswordResetEmail, sendResetSuccessEmail, sendVerificationEmail } from "../lib/mailtrap/emails.js";
  ...
  try {
    ...
    if (userExists && !userExists.isVerified) {
      if (userExists.authType === 'google') {
        return res.status(400).json({ message: 'Account already exists with Google' });
      }
...
export const googleAuth = async (req, res) => {
  try {
    ...
    if (user.authType === 'local') {
      return res.status(400).json({ message: 'Account already exists with email' });
    }
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    if (user.authType === 'google') {
      return res.status(400).json({ success: false, message: 'This account uses Google authentication. Please login with Google.' });
    }
